...
export const Auth: React.FC<AuthProps> = ({ onSuccess, onGuestMode }) => {
  ...
  const handleAuth = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (err.message === 'Failed to fetch' && (supabase as any).supabaseUrl?.includes('placeholder')) {
        ...
        setError("Google Login is not enabled in your Supabase project. Please enable it in the Supabase Dashboard (Authentication > Providers).");
      } else {
      ...
      >
        ...
        <div className="mt-6 space-y-4">
          <button
            ...
            disabled={loading}
            className="w-full py-4 bg-white dark:bg-slate-800 border-2 border-gray-100 dark:border-slate-700 hover:border-indigo-500 dark:hover:border-indigo-500 text-gray-700 dark:text-white rounded-2xl font-bold transition-all flex items-center justify-center gap-3 shadow-sm disabled:opacity-50"
          >
          ...
          <button
            onClick={onGuestMode}
            disabled={loading}
            className="w-full py-4 bg-gray-50 dark:bg-slate-800 border-2 border-transparent hover:border-gray-200 dark:hover:border-slate-600 text-gray-500 dark:text-gray-400 rounded-2xl font-bold transition-all flex items-center justify-center gap-2"
          >
            Continue as Guest
          </button>
        </div>
