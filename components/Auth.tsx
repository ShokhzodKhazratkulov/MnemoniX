    ...
    try {
      ...
      if (isSignUp) {
        ...
        if (data.session) {
          ...
          setMessage('Success! Please check your email for a confirmation link. You must confirm your email before you can sign in.');
          setIsSignUp(false);
        }
      } else {
  ...
  return (
    <div className="max-w-md w-full mx-auto p-8 bg-white dark:bg-slate-900 rounded-3xl shadow-2xl border border-gray-100 dark:border-slate-800">
      ...
      <form onSubmit={handleAuth} className="space-y-4">
        ...
        {message && (
          <div className="p-3 bg-emerald-50 dark:bg-emerald-900/20 text-emerald-600 dark:text-emerald-400 text-sm rounded-xl font-medium">
            {message}
          </div>
        )}
        ...
        >
          {loading ? <Loader2 className="animate-spin" size={20} /> : (isForgotPassword ? 'Send Reset Link' : (isSignUp ? 'Sign Up' : 'Sign In'))}
        </button>
        ...
        {isForgotPassword && (
          <button 
            ...
            className="w-full text-center text-sm font-bold text-gray-500 dark:text-gray-400 hover:text-indigo-600 transition-colors"
          >
            Back to Sign In
          </button>
        )}
